...
def _strip_volatile(schema: dict) -> dict:
    """Remove fields that may legitimately differ between runs (e.g. version)."""
...
def test_openapi_schema_matches_committed():
    ...
    assert _COMMITTED_SCHEMA.exists(), f"Committed schema not found: {_COMMITTED_SCHEMA}"
    ...
    assert result.returncode == 0, (
        f"manage.py spectacular failed:\n{result.stderr}"
    )
    ...
    assert live == committed, (
        ...
        "  DJANGO_SETTINGS_MODULE=ngcn_workbench.test_settings "
        "python3 manage.py spectacular --file "
        + str(_COMMITTED_SCHEMA)
    )
