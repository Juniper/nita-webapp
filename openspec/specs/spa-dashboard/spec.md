# SPA Dashboard Specification

## Purpose
The React SPA landing page at `/` presents a scoped, self-refreshing feed of the
most recent action-history runs, with deep links into each run's network history.
It is a tripwire — enough to reveal that something needs attention — not a
complete report; full history remains available per network.
