...
- AND no stack trace or internal URL is included in the response body

#### Scenario: Jenkins returns 403 returns 503
- GIVEN Jenkins is running but returns 403 (anonymous access not configured)
- WHEN POST /api/v1/networks/{id}/trigger/{action_id}/ is called
- THEN a 503 response is returned with `{"error": "Jenkins service unavailable"}`

### Requirement: Trigger Returns 404 for Unknown Action
The system SHALL return a `404 Not Found` response when the specified action_id
does not exist or does not belong to the network's type.

#### Scenario: Unknown action returns 404
- GIVEN an action_id that does not exist in the database
- WHEN POST /api/v1/networks/{id}/trigger/{action_id}/ is called
