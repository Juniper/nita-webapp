# Tasks: spa-network-detail-page

## 1. Replace NetworkDetailPage stub

**File**: `frontend/src/pages/NetworkDetailPage.tsx`

Replace the entire stub with the full implementation.

### 1.1 Add type definitions at the top of the file
...
**Cleanup EventSource on unmount** (handled inline in trigger handler with a ref or closure).
...
const handleTrigger = (action: Action) => {
  ...
  apiFetch(`/api/v1/networks/${id}/trigger/${action.id}/`, { method: 'POST' })
    .then(r => r.json())
    .then(d => {
      const histId: number = d.action_history_id
...
return (
  <AppLayout>
    <div className="p-6 max-w-6xl mx-auto">
      {/* Back link */}
      <Link to="/networks" className="text-indigo-400 hover:text-indigo-300 text-sm mb-4 inline-block">← Networks</Link>
...
- [ ] Network name, type, and status display in header on `/networks/:id`
- [ ] Workbook tab shows sheet summary or empty-state message
- [ ] Upload button sends multipart POST, reloads workbook on success
- [ ] Download button triggers browser file download
- [ ] Clear shows inline confirm, sends DELETE, empties sheet list
- [ ] Actions tab lists actions grouped by category
- [ ] Trigger button sends POST, console pane opens, SSE lines appear
- [ ] Console auto-scrolls; "done" label when stream closes
- [ ] History tab shows action history table
- [ ] `npm run build` passes with no TypeScript errors
- [ ] All 95 Django tests still pass
